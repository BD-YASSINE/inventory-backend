...
if (!isset($data->user_id, $data->password)) {
    send_json_response(["success" => false, "message" => "User ID and password required."], 400);
    exit;
}
...
try {
    ...
    // Delete user account
    $delStmt = $db->prepare("DELETE FROM users WHERE id = :user_id");
    $delStmt->bindParam(':user_id', $user_id, PDO::PARAM_INT);

    if ($delStmt->execute()) {
        send_json_response(["success" => true, "message" => "Account deleted successfully."]);
    } else {
        send_json_response(["success" => false, "message" => "Failed to delete account."], 500);
    }
